...
const StoreContextProvider = ({ children }) => {
  ...
  const updateUserProfile = async (uid, userData) => {
    try {
      setAuthError(null);
      const response = await api.get(`/user/${uid}`, userData);
  ...
  const submitPersonalInfo = async (personalData, uid) => {
    try {
      ...
      const payload = {
        ...
        // state_id: personalData.state_id ? Number(personalData.state_id) : null,
        dob: personalData.dob,
        // lga_id: personalData.lga_id ? Number(personalData.lga_id) : null,
        marital_status: personalData.maritalStatus,
        nin: Number(personalData.nin),
        bvn: Number(personalData.bvn),
      };
  ...
  const submitEmploymentInfo = async (employmentData, uid) => {
    try {
      ...
      const payload = {
        ...
        state_id: employmentData.state_id || null,
        lga_id: lgaId,
    ...
    try {
      ...
      switch (documentType) {
        ...
        case "utilityBill":
          ...
          break;
        case "staffId": // renamed for consistency
      ...
      const response = await uploadApi.post(endpoint, formData, {
        onUploadProgress: (progressEvent) => {
          ...
          console.log(`${documentType} upload progress: ${percent}%`);
        },
      });
  ...
  const submitCompleteForm = async (formData) => {
    try {
      ...
      // Step 4: Upload Documents (parallel but safe)
      ...
      if (formData.staffId) {
        // 🔥 make sure this matches your uploadDocument
        documentUploads.push(uploadDocument(formData.staffId, "staffId", uid));
      }
      ...
      const results = await Promise.allSettled(documentUploads);
